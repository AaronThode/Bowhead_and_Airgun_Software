...
try
    ...
    for Idate=1:length(goodFile)
        ...
        for Istation=1:length(goodFile{Idate})
            ...
            [Imiss,Ifalse,miss_diff,Itrue,SNR_miss,SNR_true]=find_similar_elements_ovlap_timelimit(best_calls.ctime,best_calls.duration,manual.individual{Idate}.ctime(:,Iletter),...
...
function [Imiss,Ifalse,miss_diff,Itrue,SNR_miss,SNR_true]=find_similar_elements_ovlap_timelimit(auto_times,auto_durations,correct_times,correct_durations, correct_SNR,run_options)
